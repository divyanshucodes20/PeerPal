...
const MyRoommateRequests: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch user's roommate requests
    // This is where you'd typically make an API call
    setRequests([
      { id: 1, location: "University Hostel A", description: "Looking for a quiet and studious roommate" },
      { id: 2, location: "Downtown Apartment", description: "Seeking a roommate to share a 2-bedroom apartment" },
    ]);
  }, []);

  const handleEdit = (request: RoommateRequest) => {
  ...
  return (
    <Container>
      <Title>My Roommate Requests</Title>
      <RequestList>
        {requests.map(request => (
          <RequestItem key={request.id}>
            <RequestInfo>
              <RequestTitle>{request.location}</RequestTitle>
